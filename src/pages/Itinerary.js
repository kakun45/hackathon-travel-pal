...
const Itinerary = ({ itineraryList, setItineraryList }) => {
  ...
  return (
    <main className="itinerary">
      ...
      <div className="itinerary__nearYou">
        You have {itineraryList.length || 0}{" "}
        {`${itineraryList.length > 1 ? "events" : "event"}`} near you
      </div>
      <img
        className="itinerary__pin"
        src={
          itineraryList.length === 2
            ? two
            : itineraryList.length === 3
            ? three
            : itineraryList.length === 4
            ? four
      ...
      <div className="itinerary__btn" onClick={sendResult}>
        <Button className="itinerary__btn" value={"Explore"} />
      </div>
    </main>
  );
};
